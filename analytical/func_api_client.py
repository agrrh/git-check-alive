...
async def parsing_version(data):
    ...
    for release in data[1:]:
        ...
        if len(data) == 100:
            logger.error(
                f"ERROR! Не найдено версии (100 записей), "
...
async def pull_request_analytics(data):
    ...
    for pullrequest in data:
        if pullrequest["closed"] and bool(pullrequest["closedAt"]):
            if (
                await to_date(pullrequest["closedAt"]) + timedelta(60)
                > datetime.utcnow()
            ):
                duration_pullrequest.append(
                    await to_date(pullrequest["closedAt"])
                    - await to_date(pullrequest["publishedAt"])
                )
                count_closed_pr += 1
...
async def json_error_401(repository_owner, repository_name, e_data):
    logger.error(
        f'E401! Ошибка токена, обращались к "{repository_owner}/{repository_name}", e_data="{e_data}".'
    )
